...
### Using several instances of the service

The Exoscale DBaaS Service can be configured using several arguments (name, type, plan, zone). This allow to use multiple instances of the Service and have each one creating a database of a specific flavor.

For example the following Acornfile defines 2 instances of the exo-dbaas service:
- the first triggers the creation of a *redis* database
- the second one creates a *postgres* database
...
containers: app: {
  ...
  entrypoint: ["/bin/sh", "-c", "/check-db.sh"]
  env: {
    URI_PG: "@{services.exo-pg.data.proto}://@{services.exo-pg.secrets.db-creds.username}:@{services.exo-pg.secrets.db-creds.password}@@{services.exo-pg.address}:@{services.exo-pg.ports}"
    URI_REDIS: "@{services.exo-redis.data.proto}://@{services.exo-redis.secrets.db-creds.username}:@{services.exo-redis.secrets.db-creds.password}@@{services.exo-redis.address}:@{services.exo-redis.ports}"
  }
  files: "/check-db.sh": """
...
This Acornfile also defines a simple container which only shows the connection strings generated by the Services.

Running this Acorn will then create 2 databases as we can see in the Exoscale dashboard

![Multipass databases](./images/multiple.png)